...
import { DetectedPattern } from "./patterns.js";
...
export interface DNAEvolutionProposal {
  targetFile: 'SOUL.md' | 'USER_MODEL.md' | 'USER.md' | 'TOOLS.md';
  section: string;
...
export class DNAEvolver {
  private proposals: DNAEvolutionProposal[] = [];

  /**
   * Generate evolution proposals from detected patterns
   */
  async generateProposals(patterns: DetectedPattern[]): Promise<DNAEvolutionProposal[]> {
    this.proposals = [];
  ...
  private async handlePreferencePattern(pattern: DetectedPattern): Promise<void> {
  ...
  private async handleRepetitionPattern(pattern: DetectedPattern): Promise<void> {
  ...
  private async handleTemporalPattern(pattern: DetectedPattern): Promise<void> {
  ...
  private async handleKnowledgeGapPattern(pattern: DetectedPattern): Promise<void> {
    this.proposals.push({
      targetFile: 'SOUL.md',
  ...
  private async handleWorkflowPattern(pattern: DetectedPattern): Promise<void> {
    this.proposals.push({
      targetFile: 'TOOLS.md',
   ...
   */
  async applyProposals(proposals: DNAEvolutionProposal[]): Promise<EvolutionResult> {
    const applied: DNAEvolutionProposal[] = [];
   ...
   */
  getEvolutionSummary(proposals: DNAEvolutionProposal[]): string {
    ...
    return `\n## 🧬 DNA Evolution Proposals\n` +
      proposals.map(p => 
        `- **${p.targetFile}** → ${p.section} (confidence: ${(p.confidence * 100).toFixed(0)}%)`
      ).join('\n') +
