...
import { HologramStore } from "./hologram.js";
import { createPatternDetector } from "./patterns.js";
import { dnaEvolver, DNAEvolutionProposal } from "./evolver.js";
...
interface EvolutionThresholds {
  minConfidence: number;      // Minimum pattern confidence to trigger
  minPatterns: number;        // Minimum number of patterns
  cooldownHours: number;      // Hours between auto-evolutions
  maxProposalsPerEvolution: number; // Max proposals to apply at once
}
...
interface EvolutionState {
  lastAutoEvolution: string | null;
  totalAutoEvolutions: number;
  ...
  rejectedProposals: Array<{
    timestamp: string;
    reason: string;
    proposals: DNAEvolutionProposal[];
  }>;
}
...
export class AutoEvolutionEngine {
  private thresholds: EvolutionThresholds;
  private state: EvolutionState = DEFAULT_STATE;
  private store: HologramStore;
  private detector: ReturnType<typeof createPatternDetector>;

  constructor(store: HologramStore, thresholds: Partial<EvolutionThresholds> = {}) {
    this.store = store;
  ...
  private isInCooldown(): boolean {
    if (!this.state.lastAutoEvolution) return false;
  ...
  async updateThresholds(newThresholds: Partial<EvolutionThresholds>): Promise<void> {
  ...
  getThresholds(): EvolutionThresholds {
...
export function createAutoEvolutionEngine(store: HologramStore): AutoEvolutionEngine {
