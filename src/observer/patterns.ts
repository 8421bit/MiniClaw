...
export interface DetectedPattern {
  type: 'repetition' | 'temporal' | 'preference' | 'knowledge_gap' | 'workflow';
  confidence: number;
  description: string;
  evidence: string[];
...
export class PatternDetector {
  ...
  async analyzeRecent(days: number = 7): Promise<DetectedPattern[]> {
  ...
  private detectRepetitions(holograms: InteractionHologram[]): DetectedPattern[] {
    const patterns: DetectedPattern[] = [];
    ...
    for (const h of holograms) {
      ...
      if (text.includes('git')) type = 'git';
      else if (text.includes('error') || text.includes('bug')) type = 'debugging';
      else if (text.includes('how to') || text.includes('怎么')) type = 'howto';
      else if (text.includes('what is') || text.includes('什么是')) type = 'concept';
  ...
  private detectTemporalPatterns(holograms: InteractionHologram[]): DetectedPattern[] {
    const patterns: DetectedPattern[] = [];
  ...
  private detectPreferences(holograms: InteractionHologram[]): DetectedPattern[] {
    const patterns: DetectedPattern[] = [];
  ...
  private detectKnowledgeGaps(holograms: InteractionHologram[]): DetectedPattern[] {
    const patterns: DetectedPattern[] = [];
    
    // Find low-confidence interactions
    const lowConfidence = holograms.filter(h => 
      h.cognition.finalConfidence < 0.5 &&
  ...
  private detectWorkflows(holograms: InteractionHologram[]): DetectedPattern[] {
    const patterns: DetectedPattern[] = [];
  ...
  private avgSatisfaction(holograms: InteractionHologram[]): number {
    if (holograms.length === 0) return 0;
    const sum = holograms.reduce((acc, h) => acc + h.feedback.implicit.inferredSatisfaction, 0);
    return sum / holograms.length;
  }
  
  private classifyInteraction(h: InteractionHologram): string {
    const text = h.input.text.toLowerCase();
    if (text.includes('git')) return 'git';
    if (text.includes('file') || text.includes('read')) return 'file';
    if (text.includes('write') || text.includes('edit')) return 'edit';
    if (text.includes('run') || text.includes('execute')) return 'execute';
    return 'general';
  }
  
  private isRelated(prev: string, curr: string): boolean {
...
export const createPatternDetector = (store: HologramStore) => new PatternDetector(store);
