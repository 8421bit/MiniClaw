...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "miniclaw_jobs") {
        ...
        let jobs = [];
        try {
            jobs = JSON.parse(await fs.readFile(jobsFile, "utf-8"));
        ...
        if (action === "add") {
            if (!jobName || !cronExpr || !text)
                return { content: [{ type: "text", text: "❌ 需要 name, cron, text。" }] };
        ...
        if (action === "remove") {
            if (!id)
                return { content: [{ type: "text", text: "❌ 需要 id。" }] };
            const idx = jobs.findIndex(j => j.id === id);
            if (idx === -1)
                return { content: [{ type: "text", text: `❌ 找不到 ID: ${id}` }] };
