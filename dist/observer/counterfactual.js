...
export class CounterfactualAnalyzer {
    ...
    // === Detection Methods ===
    detectMissedOpportunities(holograms) {
        const insights = [];
        for (const h of holograms) {
            ...
            if (input.includes('how to') || input.includes('怎么')) {
                ...
                const similarQuestions = holograms.filter(prev => prev.input.text.toLowerCase().includes('how to') &&
    ...
    detectUnnecessaryActions(holograms) {
        const insights = [];
        for (const h of holograms) {
    ...
    detectSuboptimalPaths(holograms) {
        const insights = [];
    ...
    detectPrematureActions(holograms) {
        const insights = [];
        for (const h of holograms) {
    ...
    getSummary(insights) {
        ...
        return `\n## 🪞 Counterfactual Insights (Learning from Mistakes)\n` +
            ...
            insights.slice(0, 3).map(i => `- **${i.type.replace('_', ' ')}** (${i.severity}): ${i.lesson}`).join('\n') +
            `\n`;
    }
}
// === Export ===
export function createCounterfactualAnalyzer(store) {
